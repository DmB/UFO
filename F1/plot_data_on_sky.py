...
def ICRSCoordinates(ra,dec,unit):
        if astropy.__version__ == "0.3":
            return coordinates.ICRSCoordinates(ra=ra,dec=dec,unit=(unit[0],unit[1]))
        else:
            return coordinates.SkyCoord(ra=ra, dec=dec, unit=(unit[0],unit[1]), frame='icrs')
...
def coordinates_os_stars(path):
    ...
    for line in fop.readlines():
        if line.startswith('#'):
            continue
        sl = line.split()
        c = ICRSCoordinates(ra=sl[0],dec=sl[1],unit=(units.degree,units.degree))
        x = c.ra.degree
        y = c.dec.degree
        P = 100*float(sl[8])
        theta = float(sl[10])
...
def segments_on_map(fitsfile, ra, dec, Ps, pas, scale = 1000):
        ...
        fig.show_lines(linelist1, layer='line', color='r', linewidths=0.5)
