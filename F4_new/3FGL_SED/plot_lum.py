...
def plot(UFO,Fermi):
    ...
    #for ferm in Fermi:
    #for ferm in Fermi:
        #ax_flux.errorbar(ferm.freq,ferm.nuFnu,yerr=[ferm.nuFnuerr,ferm.nuFnuerr],markersize=2,capthick=0,fmt='o',color='r')
    ...
    for line in fop.readlines():
        ...
        x=const.c.cgs.value * 1e8/float(sl[1])
        y=float(sl[2])
        if sl[4]!='nan':
            ax_flux.errorbar(x,y,yerr=[[yerr_minus],[yerr_plus]],color='red',markersize=2,capthick=0,fmt='o')
        else:
            ax_flux.errorbar(x,y,yerr=yerr_plus,lolims=True,color='red',markersize=2,capthick=0,fmt='o')
    fop.close
    ...
    plt.savefig('SED.png',bbox_inches='tight',dpi='figure')
